...
async function callGemini(message: string, history: { role: string; content: string }[], systemPrompt?: string): Promise<{ text: string; tokensUsed: number }> {
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ systemInstruction: { parts: [{ text: baseInstruction }] }, contents, generationConfig: { temperature: 0.7, maxOutputTokens: 512 } }) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error((e as any)?.error?.message || HTTP ); }
      const data: any = await res.json();
      const reply = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
      if (reply) { return { text: reply, tokensUsed: data.usageMetadata?.totalTokenCount || Math.round(reply.length / 4) }; }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!message || typeof message !== 'string') return res.status(400).json({ error: "Missing 'message' field" });
  try {
    ...
    return res.json({ message: Disculpa, hubo un inconveniente: ., tokensUsed: 0 });
  }
}
