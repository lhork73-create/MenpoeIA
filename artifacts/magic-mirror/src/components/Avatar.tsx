...
// Lazy-load the 3D avatar so Three.js doesn't block initial render
const RealisticFaceAvatar = lazy(() =>
  import('./RealisticFaceAvatar').then(m => ({ default: m.RealisticFaceAvatar }))
);

export type AvatarMode = 'particle' | 'realistic';
...
interface Particle { fx: number; fy: number; fzBase: number; region: Region; phase: number; }
...
function buildParticles(): Particle[] {
  ...
  for (let px = -FW; px <= FW; px += STEP) {
    for (let py = -FH; py <= FH; py += STEP) {
      const chinStart = 65;
      const xRadius = py > chinStart ? FW * (1 - ((py - chinStart) / (FH - chinStart)) * 0.44) : FW;
...
// ── Particle Avatar (canvas-based) ───────────────────────────────────────────
function ParticleAvatar({ status, mouthOpenAmount, micLevel = 0, theme }: AvatarProps) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (brl.blinking && brl.phase > 0.22) { brl.blinking = false; brl.phase = 0; brl.nextBlink = 2.5 + Math.random() * 5; }
      ...
      for (const { p, sx, sy, fz } of projected) {
        let brightness = 0.18 + fz * 0.88, size = 0.9 + fz * 3.0;
        let cr = pr, cg = pg, cb = pb;
        switch (p.region) {
          case 'leftBrow': case 'rightBrow': brightness *= 1.1; size *= 1.05; break;
          case 'nose': brightness *= 0.68; size *= 0.8; break;
          case 'upperLip': case 'lowerLip':
            cr = Math.min(255, pr + 10); cg = Math.min(255, pg + 15);
            brightness *= status === 'speaking' ? 1.2 + mouthOpenAmount * 0.4 : 0.75;
            break;
        }
        if (status === 'listening') brightness *= (1 + Math.sin(t * 5.5 + p.phase) * 0.2) * (1 + micLevel * 0.55);
        if (status === 'thinking') {
          const dist = Math.sqrt(p.fx ** 2 + p.fy ** 2);
      ...
      for (const { fx, fy } of eyeDefs) {
        ...
        const fz2 = 0.85 * 115;
        const x2 =  fx * cosY + fz2 * sinY;
        const z2 = -fx * sinY + fz2 * cosY;
      ...
      // Rings
      const ringAlpha = status === 'thinking' ? 0.28 : status === 'listening' ? 0.38 + micLevel * 0.25 : status === 'speaking' ? 0.30 : 0.16;
      ...
      ctx.beginPath();
      ctx.arc(CX, CY, (245 + (micLevel ?? 0) * 25) * sc, 0, Math.PI * 2);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(CX, CY, (270 + (micLevel ?? 0) * 35) * sc, 0, Math.PI * 2);
      ctx.stroke();
      ctx.setLineDash([]);
...
// ── Main Avatar — delegates to particle or realistic ─────────────────────────
export function Avatar({ status, mouthOpenAmount, micLevel = 0, theme, mode = 'particle', gender = 'female' }: AvatarProps) {
  if (mode === 'realistic') {
    return (
      <div className="relative w-full h-full">
        <Suspense fallback={
          <ParticleAvatar status={status} mouthOpenAmount={mouthOpenAmount} micLevel={micLevel} theme={theme} />
        }>
          <RealisticFaceAvatar
            status={status}
