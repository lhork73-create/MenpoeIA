  ...
  const handleDelete = (id: number) => {
    deleteMutation.mutate({ id }, {
      onSuccess: () => {
        ...
        toast({ title: 'Memory purged', description: 'Conversation deleted from neural banks.' });
      }
    });
  };
    ...
    <div className="min-h-screen bg-background text-foreground p-6 md:p-12 relative overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        <div className="flex items-center gap-6 mb-12">
          ...
          <h1 className="text-3xl font-mono uppercase tracking-widest text-primary glow-text">Neural Logs</h1>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 glass-panel rounded-3xl border border-white/5">
            ...
            <h3 className="text-lg font-mono text-muted-foreground uppercase tracking-wider">Memory Banks Empty</h3>
            <p className="text-sm text-muted-foreground/60 mt-2">Initiate contact to populate logs.</p>
          </div>
        ) : (
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-xs font-mono text-primary/60 uppercase tracking-wider mt-1">
                        {format(new Date(conv.createdAt), 'MMM dd, yyyy HH:mm:ss')}
                      </p>
                    </div>
                    <Button 
                      variant="ghost" 
                  ...
                  <div className="space-y-3 mt-4 border-t border-white/10 pt-4">
                    ...
                    {conv.messages.length > 3 && (
                      <p className="text-xs text-muted-foreground italic mt-2">
                        +{conv.messages.length - 3} more exchanges...
                      </p>
                    )}
                  </div>
                </motion.div>
